...
data_folder = '../SimCLR/motionsense'
input_shape = (400, 3)  # (timesteps, channels)
output_shape = 6
monitor = 'val_auc'
'''
...
# np_train_resampled = (np.load(os.path.join(data_folder, 'train_resampled_x.npy')),
...
#linear_eval_best_model_file_name = os.path.join(working_directory, subfolder, "supervised.finetuned.{val_auc:.2f}.hdf5")
...
# best model
#best_val_auc = '{:.2f}'.format(round(max(training_history.history[monitor]), 2))
#linear_eval_best_model_file_name = os.path.join(working_directory, subfolder, "supervised.finetuned.{}.hdf5".format(best_val_auc))
linear_eval_best_model_file_name = os.path.join(working_directory, subfolder, "supervised.finetuned.hdf5")
