...
def get_NT_Xent_loss_gradients(model, samples_transform_1, samples_transform_2, normalize=True, temperature=1.0,
                               weights=1.0):
    """
    ...
    with tf.GradientTape() as tape:
        ...
        loss = NT_Xent_loss(hidden_features_transform_1, hidden_features_transform_2, normalize=normalize,
...
def simclr_train_model(model, dataset, optimizer, batch_size, transformation_function, temperature=1.0, epochs=100,
                       is_trasnform_function_vectorized=False, verbose=0):
    """
    ...
    for epoch in range(epochs):
        ...
        for data_batch in tqdm(batched_dataset):
            ...
            # Forward propagation
            loss, gradients = get_NT_Xent_loss_gradients(model, transform_1, transform_2, normalize=True,
                                                         temperature=temperature, weights=1.0)
...
def evaluate_model_simple(pred, truth, is_one_hot=True, return_dict=True, only_auc=False):
    """
    ...
    if only_auc:
        ...
        test_auroc = sklearn.metrics.roc_auc_score(truth[:, 1], pred[:, 1], average="micro")
        if return_dict:
            return {
            ...
            return (test_cm, test_auroc)

    test_cm = sklearn.metrics.confusion_matrix(truth_argmax, pred_argmax)
    test_auroc = sklearn.metrics.roc_auc_score(truth[:,1], pred[:,1], average="micro")
    test_auprc_micro = sklearn.metrics.average_precision_score(truth[:,1], pred[:,1], average="micro")
    test_auprc_macro = sklearn.metrics.average_precision_score(truth[:,1], pred[:,1], average="macro")
    ...
    balanced_acc = sklearn.metrics.balanced_accuracy_score(truth_argmax, pred_argmax)
...
@tf.function
def NT_Xent_loss(hidden_features_transform_1, hidden_features_transform_2, normalize=True, temperature=1.0,
                 weights=1.0):
    """
