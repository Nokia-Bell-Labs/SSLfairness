...
def create_linear_model_from_base_model(base_model, output_shape, intermediate_layer=7, hidden_size=128, added_layers=3):
    ...
    for layer in range(added_layers-1):
        # x = tf.keras.layers.Dense(hidden_size, kernel_initializer=tf.random_normal_initializer(stddev=.01), activation='relu')(x)
        x = tf.keras.layers.Dense(hidden_size, activation='relu',
                                  kernel_regularizer=tf.keras.regularizers.l2(l=1e-2))(x)
        x = tf.keras.layers.Dropout(0.5, name='drop_{}'.format(layer))(x)  # https://sebastianraschka.com/faq/docs/dropout-activation.html
...
def create_full_classification_model_from_base_model(base_model, output_shape, model_name="TPN", intermediate_layer=7,
                                                     frozen_layers=4, first_last=False, middle=False):
    """
    ...
    x = tf.keras.layers.Dense(128, activation='relu',
                              kernel_regularizer=tf.keras.regularizers.l2(l=1e-2))(intermediate_x)
    x = tf.keras.layers.Dropout(0.6, name='drop')(x)
    x = tf.keras.layers.Dense(output_shape)(x)
    outputs = tf.keras.layers.Softmax()(x)
    ...
    if first_last:
        ...
        for layer in model.layers:
            layer.trainable = False
        for layer in model.layers[frozen_layers*2+1:]:
                layer.trainable = True

    lr = tf.keras.experimental.CosineDecay(initial_learning_rate=0.1, decay_steps=2000)
    model.compile(
        # optimizer=tf.keras.optimizers.Adam(learning_rate=0.001),
        optimizer=tf.keras.optimizers.Adadelta(lr),
        loss=tf.keras.losses.CategoricalCrossentropy(),
        metrics=[tf.keras.metrics.CategoricalAccuracy(name="categorical_accuracy"),
                 tf.keras.metrics.AUC(name="auc", summation_method="minoring"), tf.keras.metrics.Precision(name="precision"),
