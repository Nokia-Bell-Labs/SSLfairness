...
def combine_windowed_dataset(user_datasets_windowed, train_users, test_users=None, verbose=0):
    ...
    if len(train_x) == 0:
        ...
        train_id = np.concatenate(train_id).squeeze()
    ...
    if len(test_x) == 0:
        ...
        test_id = np.concatenate(test_id).squeeze()
...
def filter_none_label(X, y, ids):
    ...
    return (np.array(X[valid_mask]), np.array(y[valid_mask], dtype=int), np.array(ids[valid_mask], dtype=int))


def pre_process_dataset_composite(user_datasets, label_map, output_shape, train_users, test_users, window_size, shift,
                                  normalise_dataset=True, validation_split_proportion=0.2, verbose=0):
    """
    ...
    # Step 1
    user_datasets_windowed = get_windows_dataset_from_user_list_format(user_datasets, window_size=window_size,
                                                                       shift=shift)
    # Step 2
    train_x, train_y, train_id, test_x, test_y, test_id = combine_windowed_dataset(user_datasets_windowed, train_users)
    ...
    train_x, train_y_mapped, train_id = filter_none_label(train_x, train_y_mapped, train_id)
    test_x, test_y_mapped, test_id = filter_none_label(test_x, test_y_mapped, test_id)

    if verbose > 0:
        print("Test")
        print(np.unique(test_y, return_counts=True))
        print(np.unique(test_y_mapped, return_counts=True))
        print(np.unique(test_id, return_counts=True))
        print("-----------------")

        print("Train")
        print(np.unique(train_y, return_counts=True))
        print(np.unique(train_y_mapped, return_counts=True))
        print(np.unique(train_id, return_counts=True))
        print("-----------------")
    ...
    if validation_split_proportion is not None and validation_split_proportion > 0:
        train_x_split, val_x_split, train_y_split, val_y_split, train_ind, val_ind = sklearn.model_selection.train_test_split(train_x,
                                                                                                          train_y_one_hot,
                                                                                                          indices,
                                                                                                          test_size=validation_split_proportion,
                                                                                                          random_state=42)
        train_id_split = train_id[train_ind]
        val_id_split = train_id[val_ind]
    else:
        train_x_split = train_x
        train_y_split = train_y_one_hot
        train_id_split = train_id
        val_x_split = None
        val_y_split = None
        val_id_split = None

    if verbose > 0:
        print("Training data shape: X={}, Y={}, ID={}".format(train_x_split.shape, train_y_split.shape, train_id_split.shape))
        print("Validation data shape: X={}, Y={}, ID={}".format(val_x_split.shape, val_y_split.shape, val_id_split.shape))
        print("Testing data shape: X={}, Y={}, ID={}".format(test_x.shape, test_y_one_hot.shape, test_id.shape))

    np_train = (train_x_split, train_y_split, train_id_split)
    np_val = (val_x_split, val_y_split, val_id_split) if val_x_split is not None else None
    np_test = (test_x, test_y_one_hot, test_id)
...
def pre_process_dataset_composite_in_user_format(user_datasets, label_map, output_shape, train_users, window_size,
                                                 shift, normalise_dataset=True, verbose=0):
    """
...
def add_user_id_to_windowed_dataset(user_datasets_windowed, encode_user_id=True, as_feature=False, as_label=True,
                                    verbose=0):
    """
    ...
    for user, user_dataset in user_datasets_windowed.items():
        ...
        if verbose > 0:
            print(
                f"User {user}: id {repr(user)} -> {repr(user_id)}, data shape {data.shape} -> {data_modified.shape}, labels shape {labels.shape} -> {labels_modified.shape}")
